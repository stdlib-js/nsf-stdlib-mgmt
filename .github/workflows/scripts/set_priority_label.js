...
/**
* Removes priority labels from a list of label names.
*
* @private
...
function filterLabels( labels ) {
	...
	for ( i = 0; i < labels.length; i++ ) {
		v = labels[ i ];
		if ( v.toLowerCase().startsWith( 'priority:' ) === false ) {
			out.push( v );
		}
	}
	return out;
...
/**
* Ensures that only one priority label is applied at any one time.
*
* @private
...
* @param {string} priority - desired priority label
* @returns {Promise} promise which returns query results upon resolving
*/
async function main( github, context, core, priority ) {
	var issueNumber;
	var labels;
	var owner;
	var repo;
	...
	// Filter out existing priority labels:
	labels = filterLabels( labels );

	// Re-add the desired priority label:
