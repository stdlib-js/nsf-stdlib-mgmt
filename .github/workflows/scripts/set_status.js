...
/**
* Updates the "Status" project column for the current issue.
*
* @private
...
*/
async function main( github, context, core, projectName, status ) {
	var variables;
	var project;
	var result;
	var option;
	...
	if ( status.toLowerCase().startsWith( 'status:' ) ) {
		status = status.replace( /^status:\s*/i, '' );
	}
	option = field.options.filter( filterOptions );
	if ( option.length === 0 ) {
		core.warning( 'Unable to update field. Status label does not have a corresponding field option. Skipping further processing...' );
		return;
	}
	option = option[ 0 ];
	...
	* @returns {boolean} boolean indicating whether a option matches the desired status
	*/
